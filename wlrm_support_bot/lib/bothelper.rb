...
module BotHelper
  def self.send_message(params = {})
    ...
    begin
      ...
      SemanticLogger['bothelper'].error("something happened
  ...
  def self.help(params = {})
    ...
    permissions = [params[:type], params[:user_type]]
    case permissions
    when %w[admin admin], %w[testing admin]
      send_message(chat_id: chatid, text: MULTILANG['help'][language])
      send_message(chat_id: chatid, text: MULTILANG['help_partner'][language])
      send_message(chat_id: chatid, text: MULTILANG['help_internal'][language])
      send_message(chat_id: chatid, text: MULTILANG['help_admin'][language])
    when %w[internal wallarm], %w[internal admin], %w[admin wallarm]
      send_message(chat_id: chatid, text: MULTILANG['help'][language])
      send_message(chat_id: chatid, text: MULTILANG['help_partner'][language])
      send_message(chat_id: chatid, text: MULTILANG['help_internal'][language])
    when %w[partner wallarm], %w[partner admin], %w[partner partner]
      send_message(chat_id: chatid, text: MULTILANG['help'][language])
      send_message(chat_id: chatid, text: MULTILANG['help_partner'][language])
    else
      send_message(chat_id: chatid, text: MULTILANG['help'][language])
